...
function content(){
 ...
 // build table
 $addresses_table=new str_table(api_text("training_list-tr-unvalued"),TRUE,$GLOBALS['navigation']->filtersGet());
 $addresses_table->addHeader("&nbsp;",NULL,"16");
 $addresses_table->addHeader(api_text("training_list-th-lastname"),"nowarp",NULL,"`training_diary`.`lastname`");
 $addresses_table->addHeader(api_text("training_list-th-firstname"),"nowarp",NULL,"`training_diary`.`firstname`");
 $addresses_table->addHeader(api_text("training_list-th-address"),NULL,"100%");
 // get addresses
 ...
 foreach($addresses->results as $address){
  ...
  // build table fields
  $addresses_table->addField(api_link("training_view.php?idAddress=".$address->id,api_icon("icon-search",api_text("training_list-td-view"))),"nowarp");
